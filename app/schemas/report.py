...
class ReportReasonDto(BaseModel):
    ...
    id: int = Field(..., description="신고 사유 ID", example=1)
    code: str = Field(..., description="신고 사유 코드", example="ABUSIVE_LANGUAGE")
    description: str = Field(
        ...,
        description="신고 사유 설명",
        example="욕설, 비속어 사용 - 불쾌한 언어 또는 욕설이 포함되어 있어요",
    )
...
class CreateReportRequest(BaseModel):
    ...
    reported_user_id: int = Field(
        ..., alias="reportedUserId", description="신고당한 사용자 ID", example=2
    )
    ...
    reason: Optional[str] = Field(
        ...
        example="불쾌한 언어를 사용했습니다.",
        max_length=500,
    )
    should_block: bool = Field(
        ..., alias="shouldBlock", description="신고와 함께 차단 여부", example=True
    )
...
class ReportResponse(BaseModel):
    ...
    id: int = Field(..., description="신고 ID", example=1)
    reporter_id: int = Field(
        ..., alias="reporterId", description="신고를 수행한 사용자 ID", example=1
    )
    reported_user_id: int = Field(
        ..., alias="reportedUserId", description="신고당한 사용자 ID", example=2
    )
    ...
    blocked: bool = Field(..., description="차단 여부", example=True)
