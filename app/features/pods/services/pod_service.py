...
class PodService:
    ...
    async def delete_pod(self, pod_id: int) -> None:
        ...
        pod_owner_id = getattr(pod, 'owner_id', None)
        if pod_owner_id is not None:
            await self.crud.remove_pod_member(pod_id, pod_owner_id)
    ...
    async def _enrich_pod_dto(self, pod: Pod, user_id: Optional[int] = None) -> PodDto:
        ...
        if pod_id_val is not None:
            pod_dto.joined_users_count = await self.crud.get_joined_users_count(pod_id_val)
            pod_dto.like_count = await self.crud.get_like_count(pod_id_val)
            pod_dto.view_count = await self.crud.get_view_count(pod_id_val)
        ...
        for member in pod_members:
            ...
            if member_user_id is None:
                continue
            user = await self.db.get(User, member_user_id)
        ...
        if user_id and pod_id_val is not None:
            pod_dto.is_liked = await self.crud.is_liked_by_user(pod_id_val, user_id)
            ...
            if user_application:
                ...
                if app_user_id is not None:
                    app_user = await self.db.get(User, app_user_id)
        ...
        if pod_id_val is None:
            return pod_dto
        applications = await self.application_crud.get_applications_by_pod_id(pod_id_val)
        ...
        if pod_id_val is None:
            return pod_dto
        reviews = await self.review_crud.get_all_reviews_by_pod(pod_id_val)
