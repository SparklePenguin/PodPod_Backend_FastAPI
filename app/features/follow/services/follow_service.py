...
class FollowService:
    ...
    async def follow_user(self, follower_id: int, following_id: int) -> FollowResponse:
        ...
        if follower_id_value is None or following_id_value is None:
            raise ValueError("팔로우 정보가 올바르지 않습니다.")
    ...
    async def get_following_pods(
        ...
        for pod in pods:
            ...
            joined_users_count = await self.pod_crud.get_joined_users_count(pod_id)
            like_count = await self.pod_crud.get_like_count(pod_id)
            view_count = await self.pod_crud.get_view_count(pod_id)

            # 사용자가 좋아요했는지 확인
            is_liked = await self.pod_crud.is_liked_by_user(pod_id, user_id)
            ...
            # 후기 목록 조회
            reviews = await self.review_crud.get_all_reviews_by_pod(pod_id)
