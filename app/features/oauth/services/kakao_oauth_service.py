...
class KakaoLoginRequest(BaseModel):
    ...
    id_token: Optional[str] = Field(
        default=None, serialization_alias="idToken"
    )  # ID 토큰 값 (OpenID Connect)
    expires_in: str = Field(
        serialization_alias="expiresIn"
    )  # 액세스 토큰과 ID 토큰의 만료 시간(초)
    refresh_token: Optional[str] = Field(
        default=None, serialization_alias="refreshToken"
    )  # 사용자 리프레시 토큰 값
    refresh_token_expires_in: Optional[str] = Field(
        serialization_alias="refreshTokenExpiresIn"
    )  # 리프레시 토큰 만료 시간(초)
    scopes: List[str] = Field(
        default=[], serialization_alias="scopes"
...
class KakaoTokenResponse(BaseModel):
    token_type: str = Field(
        default="bearer", serialization_alias="tokenType"
    )  # 토큰 타입, bearer로 고정
    access_token: str = Field(
        serialization_alias="accessToken"
    )  # 사용자 액세스 토큰 값
    ...
    scope: List[str] = Field(
        default=[], serialization_alias="scope"
    )  # 인증된 사용자의 정보 조회 권한 범위
...
class KakaoOauthService:
    ...
    async def handle_kakao_callback(self, params: KakaoCallBackParam):
        """카카오 콜백 처리"""
        # 인가 코드 요청 실패 처리
        if params.error:
            raise_error(
        ...
        try:
            ...
            token_response = await self.get_kakao_token(
                code=params.code or "", redirect_uri=self.redirect_uri
            )
    ...
    async def get_kakao_token(
        ...
        if not client_id:
            raise ValueError("카카오 클라이언트 ID가 설정되지 않았습니다")
