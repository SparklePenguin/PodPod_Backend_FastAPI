...
class FollowRepository:
    ...
    async def create_follow(self, follower_id: int, following_id: int) -> Follow | None:
        """팔로우 생성 (커밋은 use_case에서 처리)"""
        # 자기 자신을 팔로우하는지 확인
        if follower_id == following_id:
            return None

        # 기존 팔로우 레코드 확인 (활성화 상태 무관)
        existing_follow = await self.get_follow_any_status(follower_id, following_id)
        if existing_follow:
    ...
    # - MARK: 사용자 관련 모든 팔로우 관계 삭제 (커밋 없음)
    async def delete_all_by_user(self, user_id: int) -> None:
        """특정 사용자와 관련된 모든 팔로우 관계 삭제 (커밋은 use_case에서 처리)"""
