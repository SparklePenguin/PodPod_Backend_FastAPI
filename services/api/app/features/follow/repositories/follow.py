...
from app.features.follow.models.follow import Follow
...
class FollowCRUD:
    ...
    async def create_follow(
        ...
        try:
            ...
            if existing_follow:
                existing_is_active = getattr(existing_follow, "is_active", None)
                if existing_is_active:
                    return existing_follow
                else:
                    # 비활성화된 팔로우가 있으면 재활성화
                    existing_follow_id = getattr(existing_follow, "id", None)
                    if existing_follow_id is not None:
                        stmt = (
                            update(Follow)
                            .where(Follow.id == existing_follow_id)
                            .values(is_active=True)
                        )
                        await self.db.execute(stmt)
                        await self.db.commit()
                        await self.db.refresh(existing_follow)
                    return existing_follow
        ...
        query = (
            ...
            .where(
                and_(
                    Follow.follower_id == user_id,
                    Follow.is_active,  # 활성화된 팔로우만
                    Pod.is_active.is_(True),  # 활성화된 파티만
    ...
    async def get_notification_status(
        ...
        if not follow:
            return None
        return getattr(follow, "notification_enabled", None)
        ...
        try:
            ...
            if follow_id is not None:
                stmt = (
                    update(Follow)
                    .where(Follow.id == follow_id)
                    .values(notification_enabled=notification_enabled)
                )
                await self.db.execute(stmt)
                await self.db.commit()
                await self.db.refresh(follow)
