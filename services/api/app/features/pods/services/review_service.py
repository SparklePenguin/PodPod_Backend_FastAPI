...
class PodReviewService:
    def __init__(self, session: AsyncSession, fcm_service: FCMService | None = None):
        ...
        self._pod_review_repo = PodReviewRepository(session)
    ...
    async def create_review(
        ...
        try:
            # 이미 작성한 후기가 있는지 확인
            existing_review = await self._pod_review_repo.get_review_by_pod_and_user(
            ...
            # 후기 생성
            review = await self._pod_review_repo.create_review(
    ...
    async def get_review_by_id(self, review_id: int) -> PodReviewDto | None:
        """ID로 후기 조회"""
        review = await self._pod_review_repo.get_review_by_id(review_id)
        if not review:
            return None
    ...
    async def get_reviews_by_pod(
        ...
        """특정 파티의 후기 목록 조회"""
        reviews, total_count = await self._pod_review_repo.get_reviews_by_pod(
            pod_id, page, size
        )
    ...
    async def get_reviews_by_user(
        ...
        reviews, total_count = await self._pod_review_repo.get_reviews_by_user(
            user_id, page, size
        )
    ...
    async def get_reviews_received_by_user(
        ...
        reviews, total_count = await self._pod_review_repo.get_reviews_received_by_user(
            user_id, page, size
        )
    ...
    async def update_review(
        ...
        # 후기 수정
        updated_review = await self._pod_review_repo.update_review(
    ...
    async def delete_review(self, review_id: int, user_id: int) -> bool:
        ...
        return await self._pod_review_repo.delete_review(review_id)

    # - MARK: 파티별 후기 통계 조회
    async def get_review_stats_by_pod(self, pod_id: int) -> dict:
        """파티별 후기 통계 조회"""
        return await self._pod_review_repo.get_review_stats_by_pod(pod_id)
        ...
        try:
            ...
            try:
                ...
                if user and user.id:
                    ...
                    user_tendency_type = (
                        str(user_tendency_type_raw)
                        if user_tendency_type_raw is not None
                        else ""
                    )
