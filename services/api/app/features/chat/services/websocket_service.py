...
class WebSocketService:
    ...
    async def handle_websocket_connection(
        ...
        # 채널 메타데이터 확인
        channel_metadata = await self.get_channel_metadata(channel_url)
        if not channel_metadata:
            await websocket.close(code=1003)
            raise ChatRoomAccessDeniedException(0, user_id)
        ...
        # WebSocket 연결
        await self.connection_manager.connect(websocket, channel_url, user_id)
