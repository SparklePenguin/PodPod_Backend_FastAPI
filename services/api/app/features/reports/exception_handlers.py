"""
Reports 도메인 전용 Exception Handler

이 모듈은 Reports 도메인의 예외를 처리하는 핸들러를 정의합니다.
...
async def cannot_report_self_handler(request: Request, exc: CannotReportSelfException):
    """CannotReportSelfException 처리: 자기 자신을 신고할 수 없는 경우"""
    logger.warning(f"Cannot report self: path={request.url.path}")
...
# 이 딕셔너리는 app/core/exception_loader.py에서 자동으로 읽어서 등록됩니다.
EXCEPTION_HANDLERS = {
    CannotReportSelfException: cannot_report_self_handler,
}
